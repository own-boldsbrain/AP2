...
async def handle_search_payment_methods(
    ...
    if not user_email:
        await updater.failed(
            message='user_email is required for search_payment_methods'
        )
        return
    if not method_data:
        await updater.failed(
            message='method_data is required for search_payment_methods'
        )
        return
    ...
    if (
        not eligible_aliases.get('payment_method_aliases')
        or len(eligible_aliases.get('payment_method_aliases', [])) == 0
    ):
        await updater.add_artifact(
            [
                Part(
...
async def handle_get_payment_method_raw_credentials(
    ...
    try:
        ...
        if not payment_method:
            await updater.failed(
                message=f'Payment method not found for token: {token}'
            )
            return
        ...
        await updater.failed(
            message=f'Error processing payment credentials: {str(e)}'
        )
        return
...
async def handle_create_payment_credential_token(
    ...
    try:
        ...
        if not user_email:
            await updater.failed(
                message='user_email is required for create_payment_credential_token'
            )
            return
        if not payment_method_alias:
            await updater.failed(
                message='payment_method_alias is required for create_payment_credential_token'
            )
            return
        ...
        if not payment_method:
            await updater.failed(
                message=f"Payment method '{payment_method_alias}' not found for user '{user_email}'"
            )
            return
        ...
        credentials_provider_url = (
            'http://localhost:8004/a2a/credentials_provider_agent'
        )
        ...
        await updater.failed(
            message=f'Error creating payment credential token: {str(e)}'
        )
        return
...
async def handle_signed_payment_mandate(
    ...
    try:
        ...
        await updater.failed(
            message=f'Error handling signed payment mandate: {str(e)}'
        )
        return
