...
from . import tools
from .remote_agents import solar_viability_agent_client
...
async def solar_viability_check(
    latitude: float, longitude: float, date: str, tool_context: ToolContext
) -> str:
  """Delegates to the solar viability agent to check solar viability."""
  message = (
      A2aMessageBuilder()
      .set_context_id(tool_context.state["shopping_context_id"])
      .add_text(
          "Calculate solar viability for the given location and date."
      )
  ...
  task = await solar_viability_agent_client.send_a2a_message(message)
  return task.summary
...
root_agent = RetryingLlmAgent(
    ...
    instruction="""
          ...
          purchase products from merchants. You can also help with solar
          viability analysis.
    ...
    %s
         ...
         Scenario 2:
         The user asks about solar viability.
         1. Use the `solar_viability_check` tool to get the analysis.
         ...
         The users ask you do to anything else.
          1. Respond to the user with this message:
             ...
             you can say 'I want to buy a pair of shoes' or 'check the solar
             viability for my location'"
