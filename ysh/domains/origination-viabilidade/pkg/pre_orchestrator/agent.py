...
ORIGINATION_API_URL = os.getenv(
    'ORIGINATION_API_URL', 'http://origination_api:8000'
)
VIABILITY_SERVICE_URL = os.getenv(
...
class PREOrchestratorAgent:
    ...
    async def classify_consumer(
        self, lead_id: str, classification_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    async def calculate_viability(
        self, viability_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    async def get_tariff_profile(self, tariff_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            url_profile = (
                f'{ANEEL_TARIFFS_URL}/tools/aneel.tariffs.profile.build'
            )
            profile_response = await self.http_client.post(
                url_profile, json={'rows': components.get('rows', [])}
            )
            ...
            return {
                'tariff_profile': {
                    'cents_per_kwh': DEFAULT_TARIFF_CENTS_PER_KWH
                }
            }
    ...
    async def generate_recommendations(
        self, lead_id: str, reco_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _create_offers(
        self, band_code: str, kwp: float, expected_kwh_year: float
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def orchestrate_pre_process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.emit_event(
                ...
                {'lead_id': lead_id, 'generation_modality': modality},
            )
            ...
            viability_data = {
                ...
                'system_loss_fraction': preferences.get(
                    'system_loss_fraction', DEFAULT_LOSSES
                ),
            ...
            telemetry['durations_ms']['viability'] = round(
                (time.time() - viability_start) * 1000, 2
            )
            ...
            hsp = (
                viability_response.get('kwh_year_per_kwp', DEFAULT_HSP * 365)
                / 365
            )
            pr = viability_response.get('pr', DEFAULT_PR)
            ...
            economics_data = {
                'kwh_year': kwh_year,
                'tariff_profile': tariff_profile.get(
                    'tariff_profile',
                    {'cents_per_kwh': DEFAULT_TARIFF_CENTS_PER_KWH},
                ),
            ...
            await self.emit_event(
                'system.sized.v1',
                {
                    ...
                    'expected_kwh_year': sizing_result.get(
                        'expected_kwh_year', 0
                    ),
                },
            )
            ...
            final_bundle = {
                ...
                'classification': {
                    ...
                    'consumer_subclass': classification_data[
                        'consumer_subclass'
                    ],
                ...
                'viability': {
                    'kwh_year_per_kwp': viability_response.get(
                        'kwh_year_per_kwp', 0
                    ),
                    'pr': viability_response.get('pr', DEFAULT_PR),
...
async def main():
    ...
    input_data = {
        ...
        'consumption_data': {'consumo_12m_kwh': 4000},
        'preferences': {
            'preferred_tier': 'T130',
            'has_roof': True,
            'multiple_ucs': False,
